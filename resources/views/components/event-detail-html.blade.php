<div style="word-wrap: break-word; white-space: normal; direction: rtl; text-align: right;">
    <div class="row mb-2 fs-6 text-dark">
        <div class="col-md-1"><i class="fa fa-info fa-lg" style="margin-right: 5px"></i></div>
        <div class="col-md-9">
            <div>
                {{ ucfirst(str_replace('-', ' ', $data->type)) }}
                @if(!empty($data->group_name) && $data->type == 'group')
                    ({{ $data->group_name }})
                @endif
            </div>
        </div>
        <div class="col-md-2">
            @if (request('mode') && request('mode') == 'create')
                <div class="d-flex gap-2 justify-content-end">
                    <i class="fa fa-edit" onclick="window.handleAction('edit', {{ $data }})" style="cursor: pointer;"></i>
                    {{-- <i class="fa fa-trash" onclick="window.handleAction('delete', '{{ json_encode(['schedule_id' => $data['schedule_id'], 'unique_id' => $data['unique_id']]) }}')" style="cursor: pointer;"></i> --}}
                    <i class="fa fa-trash" onclick="window.handleAction('delete', '{{ json_encode(['event_id' => $data['id']]) }}')" style="cursor: pointer;"></i>
                </div>
            @endif
        </div>
    </div>
    @if (in_array($data->type, ['individual', 'group', 'staff-meeting', 'parental-guidance']))
        <div class="row mb-2 fs-6 text-dark">
            <div class="col-md-1"><i class="fa fa-{{ appointmentIcon($data->type) }}"></i></i></div>
            <div class="col-md-11"><div>{{ getChildrenNamesById($data->childrens->pluck('children_id')->toArray()) }}</div></div>
        </div>
    @endif
    <div class="row mb-2 fs-6 text-dark">
        <div class="col-md-1"><i class="fa fa-calendar"></i></div>
        <div class="col-md-11">({{ $data->day }}) {{ date('H:i', strtotime($data->end_time)) }} - {{ date('H:i', strtotime($data->start_time)) }}</div>
    </div>
    <div class="row mb-2 fs-6 text-dark">
        <div class="col-md-1"><i class="fa fa-clock-o"></i></div>
        <div class="col-md-11">
            {{ $data->frequency_repeat }}
            @if ($data->frequency_repeat !== 'Weekly')
                {{ $data->frequency_repeat_at}}
            @endif
        </div>
    </div>
    <div class="row mb-2 fs-6 text-dark">
        <div class="col-md-1"><i class="fa fa-users"></i></div>
        <div class="col-md-11">{{ getUserNameByIds($data->therapistIds)}}</div>
    </div>
    <div class="row mb-2 fs-6 text-dark">
        <div class="col-md-1"><i class="fa fa-align-justify"></i></div>
        <div class="col-md-11">{{ $data->description }}</div>
    </div>
</div>