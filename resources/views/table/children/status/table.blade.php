    ...
    <tbody>
        @forelse ($statuses as $status)
            <tr class="tr-{{ $status->id }}">
                {{-- <td><input type="checkbox" name="id[]" value="{{ $status->id }}" class="checkbox"></td> --}}
                <td>{{ $status->name }}</td>
                <td>
                    <a href="{{ route('children-table.edit', $status->id) }}?type=status" class=""><i class="bx bx-edit icon"></i></a>
                </td>
            </tr>
        @empty
            <tr>
