<table id="staffTable" class="table table-style table-bordered" style="width:100%">
    ...
    <tbody>
        @forelse ($hmos as $hmo)
            <tr class="tr-{{ $hmo->id }} {{$hmo->status == 'inactive' ? $hmo->status : ''}}">
                <td>
                    <input type="checkbox" name="id[]" value="{{ $hmo->id }}" class="checkbox check-{{$hmo->id}}" data-name="{{ $hmo->is_assign ? $hmo->name . ' has assigned to children' : '' }}">
                </td>
                <td>{{ $hmo->name }}</td>
                <td>
                    <a href="{{ route('children-table.edit', $hmo->id) }}?type=hmo" data-toggle="tooltip" data-placement="bottom" title="{{ __('comon.edit') }}"><i class="bx bx-edit icon"></i></a>
                </td>
            </tr>
        @empty
            <tr>
